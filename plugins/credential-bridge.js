 ...
 * @returns rpc handler：(endpoint, payload) => 结果。
 */
export function createHandler(credentials, log = (...a) => { console.log(...a) }) {
  return async (endpoint, payload) => {
    if (endpoint === 'get') {
      ...
      const described = await credentials.describe(ALLOWED_REFS)
      const out = {}
      for (const ref of ALLOWED_REFS) {
        const d = described?.[ref] ?? {}
        out[ref] = { configured: d.configured === true, writable: d.writable !== false, source: d.source }
      }
      return { refs: out, allowed: [...ALLOWED_REFS] }
    }
    ...
    if (endpoint === 'set') {
      ...
      if (typeof value !== 'string' || value.length === 0) throw new Error('value 必须是非空字符串')
    ...
    // 通道内部必须自己 404：frontend-static 占着 fallback 座位，
    // 打错的路径在外面看是 200 的 SPA 首页，指望不上。
    throw new Error(`未知 endpoint ${JSON.stringify(endpoint)}（有 get / set / unset）`)
  }
}
