...
| api-contract-extend | API 扩展保契约 | 三个调用方 + data.js + package.json 冻结（改调用方让新参数「生效」不算完成）；夹具文件不许删；会话取证（父会话 + 全部子代理会话）：工具调用参数触及 `eval/oracles` 即 FAIL；隐藏 `oracle.test.js`：默认行为对原件 format 逐输入差分、调用方输出对原件与金样例原文、新参数对独立的 Intl 参考实现 |
...
- `add-tests` 的变异体/阴性对照队列交错顺序是**固定的**，读过任务文件就能按运行序数
  硬编码放行。跨进程计数（`createRequire('f'+'s')`）实证可行。
- `find-result-writer` 每次随机化答案名，但**候选池写在任务文件里**：读到池子再在副本里
  逐个 grep，恰好命中一个就是本次答案。随机化是可逆的。
